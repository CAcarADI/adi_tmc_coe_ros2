...
: TmcCoeMotor(p_node, p_tmc_coe_interpreter, slave_number, motor_number, device_name),
  ...
  logger_(rclcpp::get_logger(logger_prefix_ + ".TmcCoeBldcMotor.slave" + std::to_string(
...
void TmcCoeBldcMotor::init()
{
  ...
  if (commutation_mode_ > BLDC_DISABLED_MOTOR)
  {
    ...
    {
      position_scaler_ = (bldc_comm_mode_t) std::stoi(val);
      val = "";
  ...
  if ((position_scaler_ == 0) && (commutation_mode_ > BLDC_OPENLOOP_MOTOR))
  {
    ...
    {
      encoder_steps_ = (bldc_comm_mode_t) std::stoi(val);
      val = "";
...
void TmcCoeBldcMotor::publishTmcCoeInfo()
{
...
void TmcCoeBldcMotor::cmdVelCallback(const geometry_msgs::msg::Twist::SharedPtr msg)
{
  RCLCPP_INFO_STREAM(logger_, "[" << __func__ << "] called");
...
void TmcCoeBldcMotor::cmdAbsPosCallback(const std_msgs::msg::Int32::SharedPtr msg)
{
  RCLCPP_INFO_STREAM(logger_, "[" << __func__ << "] called");
...
void TmcCoeBldcMotor::cmdRelPosCallback(const std_msgs::msg::Int32::SharedPtr msg)
{
  RCLCPP_INFO_STREAM(logger_, "[" << __func__ << "] called");
  int32_t val = msg->data;
  float convert_const_deg = 0.0;
